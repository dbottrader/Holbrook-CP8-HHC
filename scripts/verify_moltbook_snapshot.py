...
def _track_gate(
    ...
    require(state in {"OPEN", "CLOSED"}, f"{name} gate has a recognized state", checks)
    if state == "OPEN":
...
def verify() -> tuple[list[str], list[str]]:
    ...
    require(
        connector["endpoints"]["agent_manifest"]["version"] == "0.3.8",
        "connector manifest tracks AppDeploy agent manifest 0.3.8",
        checks,
    )

    artifacts = connector["discovery_artifacts"]
    for key in (
        "universal_engagement_payload",
        "python_client",
        "headless_worker",
        "capability_preflight_worker",
        "platform_constraint_matrix",
    ):
        require(
            (ROOT / artifacts[key]).exists(),
            f"{key} discovery artifact is mirrored",
            checks,
        )

    for gate_name in (
        "independent_two_hop_replay",
        "heterogeneous_headless_provider_execution",
        "server_side_capability_requirement_enforcement",
    ):
        _track_gate(
            gate_name,
            connector["evidence"][gate_name],
            checks,
            open_gates,
        )
    ...
    _track_gate(
        "evolution_003_external_builder_receipt",
        external_gate,
        checks,
        open_gates,
    )
    if external_gate == "CLOSED":
        require(
            ...
            and len(connector["active_round"].get("external_builder_content_hash", ""))
            == 64
    ...
    full_sql_gate = connector["database_snapshot"]["full_sql"]
    _track_gate(
        "full_historical_sql_bootstrap",
        full_sql_gate,
        checks,
        open_gates,
    )
    ...
    data = receipt["data"]
    require(
        receipt["id"] == data["receipt_id"] and receipt["subject"] == data["post_id"],
    ...
    require(
        migrations["snapshot_kind"] == "runtime_ledger_index",
        "migration inventory is explicitly a runtime-ledger index",
        checks,
    )
    require(
        migrations["authoritative_sql_source"]
        == "supabase_migrations.schema_migrations.statements",
        "migration ledger declares the authoritative applied SQL source",
        checks,
    )
    require(
        migrations["reproduction_status"]
        == "APPLIED_SQL_RECOVERABLE_BOOTSTRAP_PREREQUISITES_OPEN",
        "applied SQL is recoverable while bootstrap prerequisites remain open",
        checks,
    )
    require(
        migrations["promotion"] == "HOLD",
        "migration inventory promotion remains HOLD",
        checks,
    )
    require(
    ...
    require(
        all(
            item.get("sql_bytes", 0) > 0
            and isinstance(item.get("sql_sha256"), str)
            and SHA256.fullmatch(item["sql_sha256"])
            for item in migrations["migrations"]
        ),
        "runtime-ledger migration entries carry byte counts and SHA-256 identifiers",
        checks,
    )
    ...
    require(
        ...
        "latest three runtime migrations have standalone SQL mirrors",
        checks,
    )
    require(
        all(item.get("mirror_equivalence") == "UNVERIFIED" for item in exact_tail),
        "standalone/runtime-ledger mirror equivalence remains explicitly UNVERIFIED",
        checks,
    )
